...
class App {
    ...
    let earthGeo = new THREE.SphereGeometry(600, 32, 16);
...
function renderChange(wx, hy) {
    let rx = THREE.MathUtils.lerp(0.001, -0.001, wx);
    let ry = THREE.MathUtils.lerp(-0.001, 0.001, hy);
	...
	q2.setFromAxisAngle( new THREE.Vector3( 0, 1, 0 ), rx );
	// camera.applyQuaternion(q);

    // camera.setRotationFromAxisAngle(new THREE.Vector3(1, 0, 0), ry);
    // camera.setFromAxisAngle(new THREE.Vector3(0, 1, 0), rx);
	// camera.setFromAxisAngle(new THREE.Vector3(1, 0, 0), Math. PI / 2 - ry);
